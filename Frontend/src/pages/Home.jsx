...
export default function Home() {
  ...
  if (isAuthenticated && user) {
    return (
      <main className="px-4 sm:px-6 lg:px-10 pt-6 sm:pt-8 pb-16 max-w-7xl mx-auto font-sans">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard
            ...
            value={firstName}
            sub={<><span className="text-emerald-500 font-bold">●</span> {user.email}</>}
            icon={User}
  ...
  // Fallback for unauthenticated state (matching the custom landing container styling)
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#060b18] p-6 font-sans text-[#f1f5f9]">
      <div className="bg-slate-900/50 backdrop-blur-md rounded-3xl border border-white/5 shadow-2xl px-12 py-14 max-w-lg w-full text-center">
        {/* Logo */}
        <div className="flex justify-center mb-7">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
            <svg width="26" height="26" fill="none" stroke="white" strokeWidth="2.4" viewBox="0 0 24 24">
              <path d="M3 3v18h18" /><path d="m19 9-5 5-4-4-3 3" />
            </svg>
          </div>
        </div>

        <p className="text-[11px] font-bold text-emerald-400 tracking-widest mb-3.5 uppercase">
          Institutional Grade
        </p>
        <h1 className="text-3xl font-extrabold text-white mb-3 tracking-tight leading-tight">
          Global Wealth Advisor
        </h1>
        <p className="text-[14px] text-slate-400 leading-relaxed mb-9">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <Link
            to="/advisor"
            className="px-7 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white text-sm font-bold no-underline shadow-lg shadow-indigo-500/20 transition-all hover:-translate-y-0.5"
          >
            Open Platform
          </Link>
          <Link
            to="/login"
            className="px-6 py-3 rounded-xl bg-white/5 text-slate-300 text-sm font-semibold no-underline border border-white/10 transition-colors hover:bg-white/10"
          >
            Sign In
          </Link>
        </div>
      ...
      <p className="text-xs text-slate-500 mt-8 tracking-wide">
        GLOBAL WEALTH MANAGEMENT © 2025 · For informational purposes only
      </p>
    </div>
  );
}